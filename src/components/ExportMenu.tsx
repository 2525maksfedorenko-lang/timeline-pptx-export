    ...
    <div ref={rootRef} className="relative flex-none">
      ...
      {isOpen && (
        <div role="menu" className={`absolute right-0 top-[calc(100%+4px)] ${MENU_SURFACE_CLASS}`}>
          <button type="button" role="menuitem" onClick={() => choose('pptx')} className={MENU_ITEM_CLASS}>
            <Presentation size={14} strokeWidth={2} aria-hidden="true" />
            PowerPoint (.pptx)
          </button>
          <button type="button" role="menuitem" onClick={() => choose('pdf')} className={MENU_ITEM_CLASS}>
