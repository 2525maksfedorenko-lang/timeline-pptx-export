  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1E2B38]/40 p-4">
      <div
        ...
        className="w-full max-w-lg rounded-lg border border-[#E5E5E1] bg-white p-6 text-left shadow-xl"
      >
        <h2 id={titleId} className="truncate text-base font-semibold tracking-tight text-[#1E2B38]">
          {item.label}
        </h2>
        ...
        <div className="mt-5 flex flex-col gap-1.5">
          ...
          <textarea
            ...
            className="resize-none rounded-md border border-[#E5E5E1] px-3 py-2 text-base leading-relaxed text-[#1E2B38] focus:border-[#2A9D90] focus:outline-none"
          />
        </div>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button
            ...
            onClick={onSave}
            disabled={!canSave}
            className="rounded-md bg-[#2A9D90] px-4 py-1.5 text-sm font-medium text-white transition-colors hover:bg-[#238277] disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-[#2A9D90]"
          >
            Save
          </button>
        </div>
