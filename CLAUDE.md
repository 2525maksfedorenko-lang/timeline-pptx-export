...
## Секреты
Никогда не читать и не выводить в терминал файлы с учётными данными:
`~/.config/gh/hosts.yml`, `~/.git-credentials`, `~/.ssh/`, любые `.env`
и всё прочее, что содержит токены, ключи или пароли. Это касается и
прямого чтения (`cat`, `Read`), и косвенного — `grep`, `ls` с выводом
содержимого, парсинга скриптом, подстановки в команду. Вывод команды
попадает в лог сессии, а токен из лога приходится отзывать.

Если задача требует доступа к GitHub (открыть PR, запушить, прочитать
issue) — просить об этом пользователя, а не искать сохранённые ключи.
`git push` по уже настроенному remote работает сам и разрешён; всё, для
чего нужен токен в явном виде, — это запрос к пользователю.
