...
| E3 | Gantt geometry stays ours: row 40, bar 32, zones 118/292/104; `BASE_PX_PER_DAY` is not touched. | **The handoff's geometry is adopted**: row 52, bar `min(34, row−16)`, list 320, columns 30/15.2/7px per day, panel 348 (the handoff's 520 expanded width went with the widen toggle). It lives in `src/gantt/geometry.ts` and `src/gantt/scale.ts`. `BASE_PX_PER_DAY` is still not touched — the screen's column width and the slides' inches-per-day are now separate numbers on purpose. |
...
| E5 | Nothing new is built from the handoff — no side panel, no critical path, no search toolbar. | **Almost all of it was built**: roll-up groups with collapse and a sub-task count; the Edit Task side panel; the inline add row; the today band, weekend tint and out-of-range shading. Four of those pieces have since been taken back out as unused — the critical-path mode and the Links switch, and then the toolbar's whole second row (search box and status chips), leaving a one-row header. The screen narrows only by a fold or a focus now. |
